...
- 月视图在全局顶栏左侧：中文年月份；其右：Lucide 图标切月（`ChevronLeft` 上个月、`Circle` 回到当前月、`ChevronRight` 下个月；`title` 保留中文提示）；Schedule-X 默认 header 隐藏
...
- 一周从周一起；界面中文
- 仅月网格；切换月份按可见网格窗口重新 `list_events`；不做**事件**拖拽改期
...
- **拖放视觉**：落点高亮（天格子 / 任务列表 / 图章列表）。光标：图章→日历为复制，任务→日历为移动。拖放完成后短时忽略 Schedule-X `onClickDate`，避免误开新建事件框。
